...
def get_coffee_ca_crt_key() -> Tuple[Any, Any]:
    crt = pkg_resources.resource_string(__name__, "coffee_ca.crt")
    key = pkg_resources.resource_string(__name__, "coffee_ca.key")
    return crt, key
...
def make_ca_cert(cert_path: Path, key_path: Path):
    ...
    subject = issuer = x509.Name(
        [
            ...
            x509.NameAttribute(NameOID.COMMON_NAME, "Coffee CA"),
            x509.NameAttribute(NameOID.ORGANIZATIONAL_UNIT_NAME, "Organic Farming Division"),
        ]
    )
    root_cert = (
...
def main():
    return make_ca_cert(Path("./coffee_ca.crt"), Path("./coffee_ca.key"))
