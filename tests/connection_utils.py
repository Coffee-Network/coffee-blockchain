...
async def disconnect_all_and_reconnect(server: CoffeeServer, reconnect_to: CoffeeServer) -> bool:
...
async def add_dummy_connection(
    server: CoffeeServer, dummy_port: int, type: NodeType = NodeType.FULL_NODE
    ...
    generate_ca_signed_cert(
        server.coffee_ca_crt_path.read_bytes(), server.coffee_ca_key_path.read_bytes(), dummy_crt_path, dummy_key_path
    )
    ssl_context = ssl_context_for_client(
        server.coffee_ca_crt_path, server.coffee_ca_key_path, dummy_crt_path, dummy_key_path
    )
...
async def connect_and_get_peer(server_1: CoffeeServer, server_2: CoffeeServer) -> WSCoffeeConnection:
    """
