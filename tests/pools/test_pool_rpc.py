...
from coffee.pools.pool_wallet_info import PoolWalletInfo, PoolSingletonState
from coffee.protocols import full_node_protocol
from coffee.protocols.full_node_protocol import RespondBlock
from coffee.rpc.rpc_server import start_rpc_server
from coffee.rpc.wallet_rpc_api import WalletRpcApi
...
class TestPoolWalletRpc:
    ...
    async def test_absorb_pooling(self, one_wallet_node_and_rpc, fee):
        ...
        # Balance stars at 6 XCF and 5 more blocks are farmed, total 22 XCF
    ...
    async def test_leave_pool(self, setup, fee, target_puzzle_hash):
        ...
        try:
            ...
            async def have_coffee():
                await self.farm_blocks(full_node_api, our_ph, 1)
                return (await wallets[0].get_confirmed_balance()) > 0

            await time_out_assert(timeout=WAIT_SECS, function=have_coffee)
