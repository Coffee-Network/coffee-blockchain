...
from coffee.cmds.init_funcs import create_all_ssl, create_default_coffee_config
...
from coffee.consensus.block_rewards import calculate_base_farmer_reward, calculate_pool_reward
from coffee.consensus.blockchain_interface import BlockchainInterface
from coffee.consensus.coinbase import create_farmer_coin, create_pool_coin, create_puzzlehash_for_pk
from coffee.consensus.condition_costs import ConditionCost
...
from coffee.consensus.deficit import calculate_deficit
...
from coffee.consensus.pot_iterations import (
    calculate_ip_iters,
...
from coffee.consensus.vdf_info_computation import get_signage_point_vdf_info
from coffee.daemon.keychain_proxy import connect_to_keychain_and_validate, wrap_local_keychain
from coffee.full_node.bundle_tools import (
    best_solution_generator_from_template,
    detect_potential_template_generator,
    simple_solution_generator,
)
from coffee.full_node.generator import setup_generator_args
from coffee.full_node.mempool_check_conditions import GENERATOR_MOD
from coffee.full_node.signage_point import SignagePoint
from coffee.plotting.create_plots import PlotKeys, create_plots
from coffee.plotting.manager import PlotManager
from coffee.plotting.util import PlotRefreshEvents, PlotRefreshResult, PlotsRefreshParameter, parse_plot_info
from coffee.server.server import ssl_context_for_server
...
from coffee.types.blockchain_format.foliage import Foliage, FoliageBlockData, FoliageTransactionBlock, TransactionsInfo
from coffee.types.blockchain_format.pool_target import PoolTarget
from coffee.types.blockchain_format.program import INFINITE_COST
...
from coffee.types.generator_types import BlockGenerator, CompressorArg
...
from coffee.util.condition_tools import ConditionOpcode
from coffee.util.config import load_config, save_config
from coffee.util.errors import Err
from coffee.util.hash import std_hash
from coffee.util.ints import uint8, uint16, uint32, uint64, uint128
from coffee.util.keychain import Keychain, bytes_to_mnemonic
from coffee.util.merkle_set import MerkleSet
from coffee.util.path import mkdir
from coffee.util.prev_transaction_block import get_prev_transaction_block
from coffee.util.vdf_prover import get_vdf_info_and_proof
...
class BlockTools:
    ...
    def __init__(
        ...
        create_default_coffee_config(root_path)
    ...
    async def setup_keys(self):
        if self.local_keychain:
            ...
            self.keychain_proxy = await connect_to_keychain_and_validate(
                self.root_path, log, user="testing-1.8.0", service="coffee-testing-1.8.0"
            )
        ...
        if len(self.pool_pubkeys) == 0 or len(self.farmer_pubkeys) == 0:
            raise RuntimeError("Keys not generated. Run `coffee generate keys`")
...
def get_plot_dir() -> Path:
    cache_path = Path(os.path.expanduser(os.getenv("COFFEE_ROOT", "~/.coffee/"))) / "test-plots"
    mkdir(cache_path)
    return cache_path
