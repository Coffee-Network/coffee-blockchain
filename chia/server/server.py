...
from coffee.protocols.protocol_state_machine import message_requires_reply
from coffee.protocols.protocol_timing import INVALID_PROTOCOL_BAN_SECONDS, API_EXCEPTION_BAN_SECONDS
from coffee.protocols.shared_protocol import protocol_version
from coffee.server.introducer_peers import IntroducerPeers
from coffee.server.outbound_message import Message, NodeType
from coffee.server.ssl_context import private_ssl_paths, public_ssl_paths
...
from coffee.util.errors import Err, ProtocolError
from coffee.util.ints import uint16
from coffee.util.network import is_localhost, is_in_network
from coffee.util.ssl_check import verify_ssl_certs_and_keys
...
class CoffeeServer:
    def __init__(
        ...
        coffee_ca_crt_key: Tuple[Path, Path],
        name: str = None,
        introducer_peers: Optional[IntroducerPeers] = None,
    ):
        # Keeps track of all connections to and from this node.
        logging.basicConfig(level=logging.DEBUG)
        self.all_connections: Dict[bytes32, WSCoffeeConnection] = {}
        self.tasks: Set[asyncio.Task] = set()

        self.connection_by_type: Dict[NodeType, Dict[bytes32, WSCoffeeConnection]] = {
            NodeType.FULL_NODE: {},
            NodeType.WALLET: {},
            NodeType.HARVESTER: {},
            NodeType.FARMER: {},
        ...
        self.coffee_ca_crt_path, self.coffee_ca_key_path = coffee_ca_crt_key
    ...
    async def garbage_collect_connections_task(self) -> None:
        ...
        while True:
            ...
            to_remove: List[WSCoffeeConnection] = []
    ...
    async def start_server(self, on_connect: Callable = None):
        ...
        if authenticate:
            ...
            ssl_context = ssl_context_for_server(
                self.coffee_ca_crt_path, self.coffee_ca_key_path, self.p2p_crt_path, self.p2p_key_path, log=self.log
            )
    ...
    async def incoming_connection(self, request):
        ...
        if peer_id == self.node_id:
            return ws
        connection: Optional[WSCoffeeConnection] = None
        try:
            connection = WSCoffeeConnection(
                self._local_type,
                ws,
                self._port,
                self.log,
    ...
    async def connection_added(self, connection: WSCoffeeConnection, on_connect=None):
    ...
    async def start_client(
        ...
        if auth:
            ...
            ssl_context = ssl_context_for_client(
                self.coffee_ca_crt_path, self.coffee_ca_key_path, self.p2p_crt_path, self.p2p_key_path
            )
        session = None
        connection: Optional[WSCoffeeConnection] = None
        try:
    ...
    def connection_closed(self, connection: WSCoffeeConnection, ban_time: int):
    ...
    async def incoming_api_task(self) -> None:
        ...
        while True:
            ...
            async def api_call(full_message: Message, connection: WSCoffeeConnection, task_id):
    ...
    async def send_to_others(
        ...
        node_type: NodeType,
        origin_peer: WSCoffeeConnection,
    ):
    ...
    def get_outgoing_connections(self) -> List[WSCoffeeConnection]:
        result = []
        for _, connection in self.all_connections.items():
    ...
    def get_full_node_outgoing_connections(self) -> List[WSCoffeeConnection]:
        result = []
    ...
    def get_full_node_connections(self) -> List[WSCoffeeConnection]:
        return list(self.connection_by_type[NodeType.FULL_NODE].values())

    def get_connections(self, node_type: Optional[NodeType] = None) -> List[WSCoffeeConnection]:
        result = []
        for _, connection in self.all_connections.items():
    ...
    async def get_peer_info(self) -> Optional[PeerInfo]:
        ...
        # Use coffee's service first.
        try:
            timeout = ClientTimeout(total=15)
            async with ClientSession(timeout=timeout) as session:
                async with session.get("https://ip.coffee.net/") as resp:
                    if resp.status == 200:
    ...
    def is_trusted_peer(self, peer: WSCoffeeConnection, trusted_peers: Dict) -> bool:
