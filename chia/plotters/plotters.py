...
from coffee.plotters.bladebit import get_bladebit_install_info, plot_bladebit
from coffee.plotters.chiapos import get_chiapos_install_info, plot_coffee
from coffee.plotters.madmax import get_madmax_install_info, plot_madmax
from coffee.plotters.install_plotter import install_plotter
...
def call_plotters(root_path: Path, args):
    # Add `plotters` section in COFFEE_ROOT.
    coffee_root_path = root_path
    ...
    if not os.path.exists(root_path):
        print(f"Creating plotters folder within COFFEE_ROOT: {root_path}")
        try:
            os.mkdir(root_path)
        except Exception as e:
    ...
    if args.plotter == "chiapos":
        plot_coffee(args, coffee_root_path)
    if args.plotter == "madmax":
        plot_madmax(args, coffee_root_path, root_path)
    if args.plotter == "bladebit":
        plot_bladebit(args, coffee_root_path, root_path)
