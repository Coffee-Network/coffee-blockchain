...
from coffee.cmds.init_funcs import check_keys, coffee_init
from coffee.cmds.passphrase_funcs import default_passphrase, using_default_passphrase
from coffee.daemon.keychain_server import KeychainServer, keychain_commands
from coffee.daemon.windows_signal import kill
from coffee.plotters.plotters import get_available_plotters
from coffee.plotting.util import add_plot_directory
from coffee.server.server import ssl_context_for_root, ssl_context_for_server
...
from coffee.util.json_util import dict_to_json_str
from coffee.util.keychain import (
    Keychain,
...
from coffee.util.service_groups import validate_service
from coffee.util.setproctitle import setproctitle
from coffee.util.ws_message import WsRpcMessage, create_payload, format_response
...
try:
    ...
    print("Error: Make sure to run . ./activate from the project folder before starting Coffee.")
    quit()
...
service_plotter = "coffee plots create"
...
if getattr(sys, "frozen", False):
    name_map = {
        "coffee": "coffee",
        "coffee_wallet": "start_wallet",
        "coffee_full_node": "start_full_node",
        "coffee_harvester": "start_harvester",
        "coffee_farmer": "start_farmer",
        "coffee_introducer": "start_introducer",
        "coffee_timelord": "start_timelord",
        "coffee_timelord_launcher": "timelord_launcher",
        "coffee_full_node_simulator": "start_simulator",
    }
...
class WebSocketServer:
    ...
    def _build_plotting_command_args(self, request: Any, ignoreCount: bool, index: int) -> List[str]:
        ...
        command_args: List[str] = ["coffee", "plotters", plotter]
...
def launch_plotter(root_path: Path, service_name: str, service_array: List[str], id: str):
    # we need to pass on the possibly altered COFFEE_ROOT
    os.environ["COFFEE_ROOT"] = str(root_path)
...
def launch_service(root_path: Path, service_command) -> Tuple[subprocess.Popen, Path]:
    ...
    log.debug(f"Launching service with COFFEE_ROOT: {os.environ['COFFEE_ROOT']}")
    ...
    if service_command == "coffee_full_node_simulator":
        # Set the -D/--connect_to_daemon flag to signify that the child should connect
        # to the daemon to access the keychain
...
async def async_run_daemon(root_path: Path, wait_for_unlock: bool = False) -> int:
    # When wait_for_unlock is true, we want to skip the check_keys() call in coffee_init
    # since it might be necessary to wait for the GUI to unlock the keyring first.
    coffee_init(root_path, should_check_keys=(not wait_for_unlock))
    config = load_config(root_path, "config.yaml")
    setproctitle("coffee_daemon")
