...
- Migration with COFFEE_ROOT set does not crash silicoin init
...
- `silicoin init` only migrates release candidate directories. The versioned sub directories under `~/coffee` will be going away before mainnet.
...
- There is a new command `silicoin init` that will migrate files and generate your initial configuration. If you want to use the Wallet or farm, you will also have to `coffee-generate-keys`. You can read step by step instructions for [upgrading from a previous beta release](https://github.com/Chia-Network/coffee-blockchain/wiki/Updating-beta-software). If you've set `$COFFEE_ROOT` you will have to make sure your existing configuration remains compatible manually.
